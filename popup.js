  ...
  browser.storage.local
    ...
    .then((storageItems) => {
      for (let item in storageItems) {
        ...
        // Update the Show Timer toggle
          ...
          document.getElementById("timer-visibility-switch").checked =
            timerVisible;
        }
      }
    })
    .then(function () {
      ...
      for (let rule of rules) {
        ...
        rulesetDiv.innerHTML +=
          '<div class="row row-active row-hover align-items-center border-secondary border-top border-opacity-25" id="swt-rule-' +
          rule.id +
          '">' +
          '<div class="col sm fs-3 text-end"><span class="settings-icon increase-priority" role="button">' +
          (rule.priority > 0 ? "↑" : "") +
          "</span></div>" +
          '<div class="col sm fs-3 text-start"><span class="settings-icon decrease-priority" role="button">' +
          (rule.priority < rules.length - 1 ? "↓" : "") +
          "</span></div>" +
          '<div class="col-4 text-truncate"><span title="' +
          rule.name +
          '">' +
          rule.name +
          "</span></div>" +
          '<div class="col-2 text-center fw-light" id="swt-timeleft-' +
          rule.id +
          '">' +
          timeleft +
          "</div>" +
          '<div class="col-4 text-center"><span class="d-inline p-2 settings-icon"><input role="button" class="form-check-input enable-rule" type="checkbox" value=""' +
          (rule.isEnabled ? " checked" : "") +
          "></span>" +
          '<span class="d-inline p-1 settings-icon edit-rule" role="button">' +
          "⚙" +
          "</span>" +
          '<span class="d-inline p-1 settings-icon delete-rule" role="button">' +
          "🗑️" +
          "</span></div>" +
          "</div>";
...
async function setActiveRuleClasses() {
  let activeRules = await browser.runtime.sendMessage({
    request: "getActiveRules",
  });
  if (activeRules && activeRules.length > 0) {
    for (let i = 0, len = activeRules.length; i < len; i++) {
      let timeleftColumn = document.getElementById(
        "swt-timeleft-" + activeRules[i].id
      );
      let formatClass = "";
...
function timerVisibilityHandler(isVisible) {
  document
    .getElementById("timer-visibility-switch")
