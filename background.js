...
browser.tabs.onUpdated.addListener(function (_tabId, changeInfo, tabInfo) {
...
function intervalSync() {
  ...
  if (diff >= UPDATE_INTERVAL_SECONDS * 1000) {
    intervalLastFireDate.setSeconds(
      intervalLastFireDate.getSeconds() + Math.floor(diff / 1000)
    );
    countdownTimer();
  }
}
  ...
  function createOrUpdateTimerUI(timeLeftFormatted) {
    ...
    if (!document.getElementById("swt-timer")) {
      ...
      browser.storage.local
        ...
        .then(() => {
          browser.storage.local.get("timerVisible").then((response) => {
            let visibility = true;
            if (!isEmptyObj(response)) {
  ...
  browser.scripting.executeScript({
    func: () => {
      if (!document.getElementById("swt-blocking-overlay")) {
        ...
        blockingOverlayDiv.innerHTML =
          '<div class="swt-blocking-overlay-text">You have exceeded your daily allocated time for this website.<br />Time to be productive!</div>';
...
async function storeTimerPosition(right, top) {
  await browser.storage.local.set({
    timerPosition: { right: right, top: top },
  });
}
