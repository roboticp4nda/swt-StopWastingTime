...
document
  ...
  .addEventListener("click", () => openSettings("Add New Rule"));
document
...
document
  ...
  .addEventListener("change", () => (changesMade = true));
...
async function editRule(ruleName, allocatedTime, blockList, exceptList) {
  ...
  if (rule.timeAllocatedMinutes != allocatedTime) {
    ...
    rule.timeLeftSeconds = Math.max(
      rule.timeLeftSeconds - (rule.timeAllocatedMinutes - allocatedTime) * 60,
      0
    );
...
function confirmAction(action) {
  ...
  if (action === "cancel") {
    return window.confirm(
      "You have unsaved changes! Do you still want to abort?"
    );
  }
